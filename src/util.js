...
class Util {
  ...
  constructor(sketchFile) {
    ...
    this.sketchData = {
      ...
      // sketch文件名
      sketchName: path.basename(sketchFile),
      // 页面id（为了保证页面显示顺序）
      pageOrder: [],
      // 页面数据
      pageData: {
  ...
  *getArtBoardData() {
    const pages = yield this.getPages();

    for (const page of pages) {
      // const { layers, do_objectID, name } = page;
      // const artboards = layers.filter(layer => {
      //   if (
      //     layer._class === 'artboard' || layer['<class>'] === 'MSDocumentGroup'
      //   ) {
      //     // Is artboard: json || native obj
      //     layer._parentObjectID = do_objectID; // Mark parent page's object ID
      //     return true;
      //   }
